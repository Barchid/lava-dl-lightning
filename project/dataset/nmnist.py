...
class NMNISTDataModule(LightningDataModule):
    def __init__(self, data_dir: str = 'data/nmnist', batch_size: int = 32, sampling_time: int = 1, sampling_length: int = 300, transform=augment, **kwargs):
        super().__init__()
        self.data_dir = data_dir
        self.transform = transform
        self.batch_size = batch_size
        self.sampling_time = sampling_time
        self.sample_length = sampling_length
    ...
    def prepare_data(self):
        ...
        NMNISTDataset(self.data_dir, train=True, download=True,
                      sampling_time=self.sampling_time, sample_length=self.sample_length)
        NMNISTDataset(self.data_dir, train=False, download=True,
                      sampling_time=self.sampling_time, sample_length=self.sample_length)

    def setup(self, stage=None):
        self.train_dataset = NMNISTDataset(self.data_dir, train=True, transform=self.transform,
                                           sampling_time=self.sampling_time, sample_length=self.sample_length)

        self.val_dataset = NMNISTDataset(self.data_dir, train=False, transform=self.transform,
                                         sampling_time=self.sampling_time, sample_length=self.sample_length)
...
slayer.utils.Assistant
slayer.classifier.Rate
